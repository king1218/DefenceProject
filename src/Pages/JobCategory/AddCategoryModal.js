...
const AddCategoryModal = ({refetch}) => {
    ...
    const onSubmit =async data => {
        ...
        .then(imgData => {
            ...
            const Category= {
                ...
                fetch(`https://quick-solution-server.up.railway.app/category/add`, {
                    method: 'PUT',
                    headers: {
    ...
    return (
        <div>
            <input type="checkbox" id="AddCategory-Modal" className="modal-toggle" />
           <div className='modal modal-bottom sm:modal-middle'>
          
           <div className='modal-box relative'>
           <h1 className='text-indigo-700 my-4 f text-3xl text-center'>Add Job Category</h1>
           <label htmlFor="AddCategory-Modal" className="btn btn-primary bg-indigo-700 btn-sm btn-circle absolute right-2 top-2">✕</label>
        ...
        {errors.categoryName?.type === 'required' && <span className="label-text-alt text-red-500">{errors.categoryName?.message}</span>}
             ...
             <input htmlFor='AddCategory-Modal'   className='btn btn-primary  bg-indigo-700 w-full  text-white' type="submit" value="Add Category" />
             </div>
             </form>
