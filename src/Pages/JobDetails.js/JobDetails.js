...
const {data: jobDetail,isLoading,isFetching} = useQuery(['jobs',jobid], () => fetch(`https://quick-solution-server.up.railway.app/jobs/${jobid}`)
    .then(res=>res.json())
)
...
const applyJob=()=>{
    if(jobDetail.email===User.email)
    {
        
        toast.error(`${user.displayName} You can not apply on this job beacuse you posted this job`);
    }
       ...
       navigate('/dashboard')
         ...
         if(User?.email){
          fetch(`https://quick-solution-server.up.railway.app/findjob/${jobid}?email=${User?.email}`,{
             method:'PUT',
             headers:{
                 'content-type':'application/json'
             },
             body:JSON.stringify(AppiedJob)
          })
          .then(res=>res.json())
          .then(data=>{
